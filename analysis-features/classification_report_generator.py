...
def generate_report(X, y, remind=False):
    ...
    d = {}
    d["precision"] = np.mean(cross_val_score(
        model, X, y, cv=5, scoring='precision'))
...
def forward_test(X, y, close, rows=3000, mistakes=False):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X[:-rows], y[:-rows], test_size=0.2)
    model = RandomForestClassifier()
    model.fit(X_train, y_train)
    results = model.predict(X[-rows:])
    scatter_results = list(
        map(lambda x, c: c if x == 1 else None, results, close))

    s_correct = list(map(lambda x, t, c: c if x == 1 and t ==
                     1 else None, results, y[-rows:], close))
    s_wrong = list(map(lambda x, t, c: c if x == 1 and t ==
                   0 else None, results, y[-rows:], close))
    s_missed = list(map(lambda x, t, c: c if x == 0 and t ==
                    1 else None, results, y[-rows:], close))
